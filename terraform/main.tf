...
# Allow Cloud Build SA to update instance metadata (required for gcloud compute ssh --ssh-key-file)
resource "google_compute_instance_iam_member" "cloudbuild_sa_instance_admin" {
  project       = var.project_id
  zone          = var.zone
